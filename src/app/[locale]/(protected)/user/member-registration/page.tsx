...
const MemberRegistration: React.FC = () => {
  const stepKeys = ['member', 'documents', 'success'];
  ...
  const handleNext = async () => {
    if (filteredSteps[step].key === 'documents') {
      try {
        const group_id = 1; // Replace with actual ID logic
        for (const member of members) {
          const response = await fetch(
            prefixBaseApiPath(`/common/group/member${group_id}`),
            {
              method: 'POST',
  ...
  const renderStep = () => {
    ...
    switch (key) {
          ...
          <>
            ...
            <div className="mt-4">
              <button
                type="button"
                onClick={addMember}
                className="px-4 py-2 text-white bg-blue-600 hover:bg-blue-700 font-medium rounded-md transition"
              >
                + Add Member
              </button>
            </div>
      ...
      case 'success':
        return (
          <StepComponent
            referenceId={generateReferenceId('member')}
          />
        );
    }
  };
